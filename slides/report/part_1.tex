...
\begin{equation}
	...
	\end{aligned}
\end{equation}
where $\mathbf{p}=(p_1,\dots,p_K)$ represents the mixing proportions for the clusters and each $\theta_i$ is characterized by the latent cluster $c_i$ and the corresponding parameters $\phi_{c_i}$.
...
Assuming that the current state of Markov chain is composed of $(c_1,\dots,c_n)$  and the unique values $\phi_c$ for all $c=1,\dots,k$, the Gibbs sampler should first draw a new value $c$ for each $c_i$ according to the following probabilities:
\begin{align}
	\hspace{-25pt}
	\vspace{-12pt}
...
The transition to the infinite case, that is, the reference DPM model (\ref{dpm-2}), is handled by taking the limit as $K$ goes to infinity in the conditional distribution of $c_i$, which becomes as follows:
\begin{equation}
...
Let us now introduce the actual \verb|Neal2| algorithm, which works iteratively in two steps, in which we sample $\boldsymbol c$ and $\boldsymbol\phi$, respectively.
...
If the new value of $c_i$ is different from all the other $c_j$, a value for $\phi_{c_i}$ is drawn from the \emph{posterior distribution} $H_i$, based on the prior $G_0$ and the single observation $y_i$; this means that in this case, a new cluster has been created. \\
Then, for all clusters, the sampling of their unique value $\phi_c$ is conducted by considering their \emph{posterior distribution} based on the prior and on all observations belonging to that cluster.
...
Once all the $\phi_c$ that are no longer associated with any observation are discarded, the algorithm proceeds, for each cluster, with the sampling of $\phi_c$ from the posterior computed with the observations of the specific cluster, like in \verb|Neal2|.
