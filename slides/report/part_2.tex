...
The \verb|probas| vector of weights for the new allocation value is computed, according to the probabilities showed in \ref{probasneal2}, by also using the marginal density in \verb|data[i]|, which is known to be a Student's $t$ as mentioned in section \ref{nnig}.
...
We chose the prior parameters for the Normal-NIG model \ref{???} as follows: $\mu_0 = 5, \lambda = 1, \alpha_0 = 2, \beta_0 = 2$.
...
This is consistent with the fact that the probability of creating a new cluster is proportional to $M$, as seen in \ref{???}. \\ %TODO ref
...
Indeed, the only direct impact of $m$ is in the estimation of the marginal distribution (see \ref{???}) that has a weight of $\frac{M}{M+n}$, which is negligible if $M$ is small. %TODO ref
...
As shown in \ref{???}, each of these clusters has its own density estimate, which we refer to as \emph{component}, and a weight attached to it proportional to its cardinality. %TODO ref
...
In this case the weights turn out to be approximately equal (??? and ??? respectively). %TODO values
...
One such example is given by the best clustering of \verb|Neal8| run with $M=1$ (and $m=3$ as before), found at iteration ???. %TODO value
...
\begin{itemize}
	...
	Of course this is a daunting task that is far beyond the scope of this library, although we do think it is possible to achieve with reasonable effort. %TODO togliamo?
\end{itemize}
