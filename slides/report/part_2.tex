...
\begin{itemize}
	...
	Computations involve, among other things, the \verb|card| vector, the \verb|like()| evaluated in \verb|data[i]|, and the total mass parameter.
...
In \verb|sample_allocations()|, a loop is performed over data points $i$ and the \verb|card| vector is built, just as before.
...
The convergence in mean of the algorithm grants the correctness of this least square estimate, at it is the closest approximation to the mean dissimilarity matrix.
...
As we can see, the total mass $M$ acts as a smoothing parameter and, given its strong influence on the number of mixture components, it is a prime candidate for a prior distribution being put onto it.
