...
write.table(true_clust1, "csv/test/true_clust1.csv", row.names = F, col.names = F)
...
write.table(true_clust2, "csv/test/true_clust2.csv", row.names = F, col.names = F)
...
write.table(true_clust3, "csv/test/true_clust3.csv", row.names = F, col.names = F)
...
write.table(true_clust4, "csv/test/true_clust4.csv", row.names = F, col.names = F)

d=c(2, 5, 10, 20)
for(i in seq(length(d))){
  test=rbind(rmvnorm(200, rep(-3,d[i]),  diag(d[i])),rmvnorm(200, rep(3,d[i]),  diag(d[i])))
